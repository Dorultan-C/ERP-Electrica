...
export default function HRUsersSection() {
  ...
  const columns: DataListColumn<User>[] = [
      ...
      cell: ({ data }) => (
        <>
          <Cell.Avatar
            ...
            name={data ? `${data.firstName || ''} ${data.lastName || ''}`.trim() : '?'}
            size="small"
            className="sm:hidden"
          />
          <Cell.Avatar
    ...
    },
    {
      id: 'name',
      header: 'Name',
      accessor: (user) => user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : 'Unknown User',
      sortable: true,
      searchable: true,
      cell: ({ value, data }) => (
        <Cell.Stack>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-3 sm:p-4">
        ...
        <p className="text-green-800 dark:text-green-200 text-sm">
          HR module now includes full mobile responsiveness with touch-friendly drawers,
          adaptive layouts, and optimized spacing for small screens. Drawers are full-width on mobile
          and properly sized on larger screens.
        </p>
      </div>
